...
Originalmente, el concepto de TRL (technology readiness levels) o niveles de madurez tecnológica fué acuñado por la NASA en la década de los 70, sin embargo en 2013 la Organización Internacional de Estandarización formalizó este esquema publicando la norma internacional ISO 16290:2013, titulada "Sistemas espaciales - Definición de los Niveles de Madurez Tecnológica (TRL) y sus criterios de evaluación".
...
El último componente que hay que tener en cuenta para diseñar un sistema de alerta temprana es la capacidad de respuesta, esto significa que cada tipo de alerta debe estar ligado a un plan de acción en concreto que debe llevar a cabo el personal del Ministerio Publico Fiscal. Este punto excede el alcance de este trabajo en particular dada la complejidad y especificidad de los procesos que lleva a cabo tal institución. Queda pendiente para trabajos que pudieran realizarse en el futuro evaluar el impacto del sistema de forecasting en el uso de recursos.
...
Nivel Naranja (Alerta): Se establece cuando los algoritmos determinan una alta probabilidad matemática de que las variables más significativas vulneren los umbrales de seguridad en un horizonte temporal cercano. Este nivel activa los procesos que el usuario haya predefinido o considere adecuado para esta alerta según su experiencia.

Nivel Rojo (Crítico): Representa la inminencia o la ocurrencia efectiva del evento adverso. En este nivel, el sistema pone en conocimiento al usuario de que está ante una anomalía significativa y que debe prepararse o tomar decisiones en proporción.
...
\caption{Arquitectura de Bootstrap comparada con Gradient Boosting}
\label{fig:arquitectura-de-bootstrap-comparada-con-}
\end{figure}

Fuente: Elaboración propia en base a la arquitectura de Bagging y Boosting

\subsection{Light GBM y XG Boost}\label{light-gbm-y-xg-boost}
...
Suponiendo que se dispone de un sistema con \$\$K\$\$ variables distintas medidas simultáneamente a lo largo de \$\$T\$\$ instantes de tiempo. En cada paso de tiempo \$\$t\$\$, las observaciones no son un número escalar, sino un vector de observaciones \$\$\textbackslash mathbf\{z\}\_t \textbackslash in \textbackslash mathbb\{R\}\^{}K\$\$:

\$\$\textbackslash mathbf\{z\}\_t = {[}z\_\{t,1\}, z\_\{t,2\}, \textbackslash dots, z\_\{t,K\}{]}\^{}T\$\$

El conjunto de datos completo se representa como una secuencia cronológica de vectores o, equivalentemente, como una matriz de datos original \$\$\textbackslash mathbf\{Z\}\$\$ de dimensiones \$\$T \textbackslash times K\$\$:

\$\$\textbackslash mathbf\{Z\} = \textbackslash begin\{bmatrix\} \textbackslash mathbf\{z\}\_1\^{}T\\
\textbackslash mathbf\{z\}\_2\^{}T\\
\textbackslash vdots\\
\textbackslash mathbf\{z\}\_T\^{}T \textbackslash end\{bmatrix\} = \textbackslash begin\{bmatrix\} z\_\{1,1\} \& z\_\{1,2\} \& \textbackslash dots \& z\_\{1,K\}\\
z\_\{2,1\} \& z\_\{2,2\} \& \textbackslash dots \& z\_\{2,K\}\\
\textbackslash vdots \& \textbackslash vdots \& \textbackslash ddots \& \textbackslash vdots\\
z\_\{T,1\} \& z\_\{T,2\} \& \textbackslash dots \& z\_\{T,K\} \textbackslash end\{bmatrix\}\$\$
...
\begin{longtable}[]{@{}
>{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1437}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.0985}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1211}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1211}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1211}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1211}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1211}}
  >{\centering\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1211}}@{}}
\caption{Ilustración sobre el método de retardos con ventana deslizante.}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Fecha} & \textbf{Dia} & \textbf{Temp} & \textbf{Lesiones} & \textbf{Inercia} & \textbf{Lagg\_1} & \textbf{Lagg\_3} & \textbf{Víctimas} \\
01-06-2022 & L & 24 & 0 & 0 & 0 & 0 & 0 \\
02-06-2022 & M & 24.5 & 0 & 0 & 0 & 0 & 0 \\
03-06-2022 & X & 22 & 2 & 0 & 0 & 0 & 1 \\
04-06-2022 & J & 23 & 0 & 1 & 1 & 1 & 0 \\
05-06-2022 & V & 21.7 & 3 & 0 & 0 & 1 & 3 \\
06-06-2022 & S & 20 & 2 & 2.2 & 3 & 4 & 1 \\
07-06-2022 & D & 14.8 & 3 & 2.7 & 1 & 4 & 4 \\
08-06-2022 & L & 12 & 0 & 3.1 & 4 & 8 & 0 \\
09-06-2022 & M & 16.5 & 0 & 0 & 0 & 5 & 0 \\
10-06-2022 & X & 22 & 0 & 0 & 0 & 4 & 0 \\
11-06-2022 & J & 22.2 & 0 & 0 & 0 & 0 & 0 \\
12-06-2022 & V & 21.9 & 3 & 0 & 0 & 0 & 1 \\
\end{longtable}

Fuente: Elaboración Propia

En tabla de arriba se puede observar como queda formateado un set de datos bajo la estructura de formulación autorregresiva supervisada, nótese que cada fila contiene información del presente como el día y la temperatura, e información del pasado como los retardos (laggs) y la inercia. Luego se registra la cantidad de víctimas bajo esas condiciones, el modelo que se use intentará predecir las víctimas conociendo el contexto disponible.

\subsection{Auto Gluon y Transformers}\label{auto-gluon-y-transformers}
...
La estrategia propuesta para el uso de AutoGluon no implica que un modelo único domine todos los escenarios reales posibles, es de suponer que según la naturaleza del el problema que se enfrente, las limitaciones de información que se disponga, y el nivel de certidumbre requerida condicione la selección del modelo. Es por eso que el objetivo principal de esta tecnología es proveer al usuario una multiplicidad de modelos subdivididos en tres categorías estructurales que se definen a continuación.
...
\$\$\textbackslash hat\{y\}\_\{\textbackslash text\{ensemble\}\} = \textbackslash sum\_\{m=1\}\^{}\{M\} w\_m \textbackslash cdot \textbackslash hat\{y\}\_m, \textbackslash quad \textbackslash text\{sujeto a \} \textbackslash sum w\_m = 1 \textbackslash text\{ y \} w\_m \textbackslash geq 0\$\$

Esta última ecuación muestra que la predicción final es el resultado de agregar las predicciones de cada uno de los diferentes modelos según el peso ponderado que le asigna el método de Auto Gluon.

De este modo el uso y la implementación de este modelo especializados para forecasting en series temporales no solo es un software aislado sino que permite operativizar mediante un framework de trabajo la automatización del procesamiento de datos, la alineación temporal, el uso de transformers permitiendo el ajuste fino (modificaciones específicas al modelo original del transformer) y el ensamble ponderado final.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{imagenes/image21.png}
\caption{Esquema de Ensamble Ponderado de AutoGluon (Weighted Ensemble)}
\label{fig:esquema-de-ensamble-ponderado-de-autoglu}
\end{figure}

Fuente: Elaboración propia según la documentación del repositorio de Auto-Gluon Time-Series
